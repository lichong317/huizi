...
public class TdProductCategoryService {
	...
	/**
	 * @author lc
	 * @注释：搜索商品类别 从第三级开始搜索、如果结果不为空则去除重复的第一级别类别相同的项，如果为空则搜索第二级别，
	 */
	 ...
	 * @author DengXiao
	 */
	public TdProductCategory findByParentIdAndTitle(Long parentId, String title) {
		if (null == parentId || null == title) {
			return null;
		}
		return repository.findByParentIdAndTitle(parentId, title);
	}
