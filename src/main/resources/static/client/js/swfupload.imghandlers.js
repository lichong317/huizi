$(function () {
    ...
    $.swfUpLoadDefaults.property = {
        ...
        sendurl: null, //发送地址
        filetypes: "*.jpg;*.jpge;*.png;*.txt;", //文件类型
        filesize: "1024", //文件大小
    ...
    //初始化(360度图片展示)SWFUpload上传控件
    $.fn.InitSWFUpload_show360 = function (p) {
        p = $.extend({}, $.swfUpLoadDefaults.property, p || {});
        //创建上传按钮
        var parentObj = $(this);
        var parentBtnId = "upload_span_" + Math.floor(Math.random() * 1000 + 1);
        ...
        var swfu = new SWFUpload({
            ...
            upload_progress_handler: uploadProgress,
            upload_error_handler: uploadError,
            upload_success_handler: uploadSuccess_show360,
            upload_complete_handler: uploadComplete,
    ...
    //初始化(开箱动画)SWFUpload上传控件
    $.fn.InitSWFUpload_open_flv = function (p) {
        p = $.extend({}, $.swfUpLoadDefaults.property, p || {});
        //创建上传按钮
        var parentObj = $(this);
        var parentBtnId = "upload_span_" + Math.floor(Math.random() * 1000 + 1);
        ...
        var swfu = new SWFUpload({
            ...
            upload_progress_handler: uploadProgress,
            upload_error_handler: uploadError,
            upload_success_handler: uploadSuccess_open_flv,
            upload_complete_handler: uploadComplete,
...
//文件上传的处理已经完成且服务端返回了200的HTTP状态时触发(360度图片展示)
function uploadSuccess_show360(file, serverData) {
    try {
        var jsonstr = eval('(' + serverData + ')');
        if (jsonstr.status == '0') {
            var progressObj = $(this.customSettings.upload_target).children(".upload-progress");
        ...
        if (jsonstr.status == '1') {
            ...
            if (this.customSettings.button_action == SWFUpload.BUTTON_ACTION.SELECT_FILE) {
                //$(this.customSettings.upload_target).siblings(".upload-name").val(jsonstr.name);
                $(this.customSettings.upload_target).siblings(".upload-name").val(jsonstr.name.substring(0, jsonstr.name.lastIndexOf(".")));
                $(this.customSettings.upload_target).siblings(".upload-path").val(jsonstr.path);
                $(this.customSettings.upload_target).siblings(".upload-pdfpath").val(jsonstr.pdfpath);
                ...
                addImage_show360($(this.customSettings.upload_target), jsonstr.path, jsonstr.thumb);
            }
            var progressObj = $(this.customSettings.upload_target).children(".upload-progress");
            progressObj.children(".txt").html("上传成功：" + file.name);
        }
...
//文件上传的处理已经完成且服务端返回了200的HTTP状态时触发(开箱动画)
function uploadSuccess_open_flv(file, serverData) {
    try {
        var jsonstr = eval('(' + serverData + ')');
        if (jsonstr.status == '0') {
            var progressObj = $(this.customSettings.upload_target).children(".upload-progress");
        ...
        if (jsonstr.status == '1') {
            ...
            if (this.customSettings.button_action == SWFUpload.BUTTON_ACTION.SELECT_FILE) {
                ...
                addImage_open_flv($(this.customSettings.upload_target), jsonstr.path, jsonstr.thumb);
            }
            var progressObj = $(this.customSettings.upload_target).children(".upload-progress");
            progressObj.children(".txt").html("上传成功：" + file.name);
        }
...
function addImage(targetObj, originalSrc, thumbSrc) {
    ...
    //颜色组图
    var Pics = targetObj.siblings(".pics");
    var Pics_ = Pics.val() + originalSrc + ";"
    Pics.val(Pics_);

}
//======================================360度图片处理事件======================================
//添加360度图片
function addImage_show360(targetObj, originalSrc, thumbSrc) {
    //插入到360度图片UL里面
    var newLi = $('<li>'
    + '<input type="hidden" name="hid_photo_name_show360" value="0|' + originalSrc + '|' + thumbSrc + '" />'
    + '<input type="hidden" name="hid_photo_remark_show360" value="" />'
    + '<div class="img-box">'
    + '<img src="' + thumbSrc + '" bigsrc="' + originalSrc + '" />'
    + '</div>'
    + '<a href="javascript:;" onclick="delImg(this);">删除</a>'
    + '</li>');
    newLi.appendTo(targetObj.siblings(".photo-list_show360").children("ul"));
}
//==========================================================================================

//======================================开箱动画处理事件======================================
//添加开箱动画
function addImage_open_flv(targetObj, originalSrc, thumbSrc) {
    //插入到开箱动画UL里面
    var newLi = $('<li>'
    + '<input type="hidden" name="hid_photo_name_open_flv" value="0|' + originalSrc + '|' + thumbSrc + '" />'
    + '<input type="hidden" name="hid_photo_remark_open_flv" value="" />'
    + '<div class="img-box">'
    + '<img src="' + thumbSrc + '" bigsrc="' + originalSrc + '" />'
    + '</div>'
    + '<a href="javascript:;" onclick="delImg(this);">删除</a>'
    + '</li>');
    newLi.appendTo(targetObj.siblings(".photo-list_open_flv").children("ul"));
}
//==========================================================================================
...
function delImg(obj) {
    ...
    //颜色组图
    var Pics = parentObj.parent().siblings(".pics");
    Pics.val("");

    for (var i = 0; i < parentObj.find("li .img-box").length; i++) {        
        var pic_src = parentObj.find("li .img-box").eq(i);
        var Pics = parentObj.parent().siblings(".pics");
        Pics.val(Pics.val() + pic_src.children("img").attr("src") + ";"); 
    }
