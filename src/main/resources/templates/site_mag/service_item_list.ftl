...
<body class="mainbody">
<form name="form1" method="post" action="/Verwalter/setting/service/list" id="form1">
<div>
...
<div class="toolbar-wrap">
  <div id="floatHead" class="toolbar">
    <div class="l-list">
      <ul class="icon-list">
        <li><a class="add" href="/Verwalter/setting/service/edit"><i></i><span>新增</span></a></li>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="ltable">
  ...
  <tr class="odd_bg">
    ...
    <th align="left" width="32%">描述</th>
    <th align="left" width="12%">排序</th>
    <th width="6%">是否商品服务</th>
    <th width="6%">是否启用</th>
    <th width="10%">操作</th>
  </tr>
    ...
    <#if service_item_list??>
        <#list service_item_list as item>
            <tr>
                ...
                <td><a href="/Verwalter/setting/service/edit?id=${item.id?c}">${item.title!""}</a></td>
                <td><img src="${item.logo!''}" width="40" height="40" title="${item.title!''}" /></td>
                <td>${item.description!''}</td>
                <td><input name="listSortId" type="text" value="${item.sortId!""}" class="sort" onkeydown="return checkNumber(event);"></td>
                <td align="center"><#if item.isGoodsService?? && item.isGoodsService>是<#else>否</#if></td>
                <td align="center"><#if item.isEnable?? && item.isEnable>是<#else>否</#if></td>
                <td align="center">
                    <a href="/Verwalter/setting/service/edit?id=${item.id?c}">修改</a>
                </td>
              </tr>
        </#list>
    </#if>
